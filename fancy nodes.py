...
def alphabeta_min_node(board, color, alpha, beta, limit, caching = 0, ordering = 0):
    ...
    if (ordering == 1):
        ...
        tup_of_moves = fat_list
        #tup_of_moves = []
        #for j in fat_list:
        #    tup_of_moves.append((j[1][0], j[1][1]))
        for i in tup_of_moves:
            new_board = play_move(board, new_color, i[1][0], i[1][1])
...
def alphabeta_max_node(board, color, alpha, beta, limit, caching = 0, ordering = 0):
    ...
    if (ordering == 1):
        ...
        #    tup_of_moves.append((j[1][0], j[1][1]))
        for i in tup_of_moves:
            new_board = play_move(board, color, i[1][0], i[1][1])
...
'''
    ...
    nice_board = board
    for i in tup_of_moves:
        new_board = play_move(board, color, i)
        if compute_utility(new_board, color) > max:
            max = compute_utility(new_board, color)
            nice_board = new_board
    if color == 1:
        new_color = 2
    if color == 2:
        new_color = 1       
    minimax_min_node(nice_board, new_color)
    '''
